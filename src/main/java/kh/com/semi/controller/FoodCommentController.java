...
import kh.com.semi.dao.FoodCommentDAO;
import kh.com.semi.dto.FoodCommentDTO;

@WebServlet("*.foodco")
public class FoodCommentController extends HttpServlet {
	...
	private void actionDo(HttpServletRequest request, HttpServletResponse response)
		...
		if (cmd.equals("/FoodinsertProc.foodco")) {
			...
			int rs = dao.insertComment(new FoodCommentDTO(0, nickname, comment_content, null, board_seq, id));
			if (rs == 1) {
		...
		} else if (cmd.equals("/getFoodCommentProc.foodco")) {
			System.out.println("댓글 조회 요청 도착");
			int board_seq = Integer.parseInt(request.getParameter("board_seq"));
			System.out.println("board_seq : " + board_seq);
			ArrayList<FoodCommentDTO> commentList = dao.getCommentList(board_seq);

			Gson gson = new Gson();
			String rs = gson.toJson(commentList);
			// System.out.println(rs);
			response.getWriter().write(rs);
		} else if (cmd.equals("/FooddeleteProc.foodco")) {
		...
		} else if(cmd.equals("/FoodmodifyProc.foodco")) {
