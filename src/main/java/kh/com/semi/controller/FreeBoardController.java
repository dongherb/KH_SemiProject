...
import kh.com.semi.dao.FreeFileDAO;
import kh.com.semi.dto.FreeBoardDTO;
import kh.com.semi.dto.FreeFileDTO;
import kh.com.semi.service.FreeBoardService;
...
private void actionDo(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException{
		...
		if(cmd.equals("/toBoard.freebo")) { // 데이터 받으려면 toBoard.bo 타서 가야함.
			...
			ArrayList<FreeBoardDTO> list = service.getBoardList((int)naviMap.get("currentPage"));
			System.out.println("test.jsp의 요청");
			ArrayList<FreeBoardDTO> dtoList = dao.selectAll();
			request.setAttribute("dtoList", dtoList);

			  if(list!=null) {
				RequestDispatcher rd = request.getRequestDispatcher("/board/Free/FreeBoard.jsp");
		...
		else if(cmd.equals("/toWrite.freebo")) {
			response.sendRedirect("/board/Free/FreeWrite.jsp");
		}else if(cmd.equals("/writeProc.freebo")) {
			...
			try {
				...
				int rs = dao.insert(new FreeBoardDTO(0,writer_nickname,board_title,board_content,null,0,0,board_category,id));
				int board_seq = dao.getBoardSeq();
				System.out.println("보드 시퀀스 확인입니다." + board_seq);
				
				String origin_name = multi.getOriginalFileName("file"); // input에서 넘겨주는 이름(name) 값 
				String system_name = multi.getFilesystemName("file"); // 서버에 실제로 저장되는 이름 값
				FreeFileDAO daoFile = new FreeFileDAO();
				int rsFile = daoFile.insertFile(board_seq,origin_name,system_name,folderName);
				if(rs!=-1 || rsFile !=-1) {
					response.sendRedirect("/toBoard.freebo?currentPage=1");
				}
			}catch(Exception e) {
				e.printStackTrace();
			}
		}else if(cmd.equals("/toDetailView.freebo")){
			int board_seq = Integer.parseInt(request.getParameter("board_seq"));
			int currentPage = Integer.parseInt(request.getParameter("currentPage"));
			System.out.println("게시글 번호 : " + board_seq);
			System.out.println("currentPage : " + currentPage);
			// 파일을 띄우기만 할거면 origin name은 굳이 필요가 없음. 저장된 이름만 알면 괜찮다.
			FreeFileDAO fileDAO = new FreeFileDAO();
			FreeFileDTO fileDTO = new FreeFileDTO();
			
			// 조회수 + 해주는 메서드 호출
			dao.plusView_count(board_seq);
			FreeBoardDTO dto = dao.selectBySeq(board_seq);
			if(dto!= null) {
				...
				RequestDispatcher rd = request.getRequestDispatcher("/board/Free/FreeDetailView.jsp");
				request.setAttribute("currentPage", currentPage);
				request.setAttribute("dto", dto);
				rd.forward(request,response);
			}
		}else if(cmd.equals("/deleteProc.freebo")) {
		...
		}else if(cmd.equals("/modifyProc.freebo")) {
		...
		}else if (cmd.equals("/RecommendProc.freebo")) {
