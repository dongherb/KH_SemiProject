...
import kh.com.semi.dao.FreeCommentDAO;
...
public class FreeCommentController extends HttpServlet {
	...
	private void actionDo(HttpServletRequest request, HttpServletResponse response)
		...
		if (cmd.equals("/insertProc.freeco")) {
			...
			int rs = dao.insertComment(new FreeCommentDTO(0, nickname, comment_content, null, board_seq, id));
			if (rs == 1) {
		...
		} else if (cmd.equals("/getCommentProc.freeco")) {
			System.out.println("댓글 조회 요청 도착");
			int board_seq = Integer.parseInt(request.getParameter("board_seq"));
			System.out.println("board_seq : " + board_seq);
			ArrayList<FreeCommentDTO> commentList = dao.getCommentList(board_seq);

			Gson gson = new Gson();
			String rs = gson.toJson(commentList);
			// System.out.println(rs);
			response.getWriter().write(rs);
		} else if (cmd.equals("/deleteProc.freeco")) {
		...
		} else if(cmd.equals("/modifyProc.freeco")) {
