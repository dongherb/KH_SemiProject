...
public class ExerciseCommentDAO {
	private BasicDataSource bds;

	public ExerciseCommentDAO() {
		try {
	...
	// 댓글 목록 긁어오기
	public ArrayList<ExerciseCommentDTO> getCommentList(int board_seq) {
		String sql = "SELECT * FROM tbl_comment WHERE board_seq = ?";
		try (Connection con = this.getConnection(); PreparedStatement pstmt = con.prepareStatement(sql);) {

			pstmt.setInt(1, board_seq);
			ResultSet rs = pstmt.executeQuery();
			ArrayList<ExerciseCommentDTO> list = new ArrayList<>();
			while (rs.next()) {
				...
				list.add(new ExerciseCommentDTO(comment_seq, writer_nickname, comment_content, written_date, board_seq,
						writer_id));
			}
			return list;
		} catch (Exception e) {
	...
	// 댓글 등록 메서드
	public int insertComment(ExerciseCommentDTO dto) {
