...
import kh.com.semi.dto.FoodCommentDTO;

public class FoodCommentDAO {
	private BasicDataSource bds;

	public FoodCommentDAO() {
		try {
	...
	// 댓글 목록 긁어오기
	public ArrayList<FoodCommentDTO> getCommentList(int board_seq) {
		String sql = "SELECT * FROM tbl_comment WHERE board_seq = ?";
		try (Connection con = this.getConnection(); PreparedStatement pstmt = con.prepareStatement(sql);) {

			pstmt.setInt(1, board_seq);
			ResultSet rs = pstmt.executeQuery();
			ArrayList<FoodCommentDTO> list = new ArrayList<>();
			while (rs.next()) {
				...
				list.add(new FoodCommentDTO(comment_seq, writer_nickname, comment_content, written_date, board_seq,
						writer_id));
			}
			return list;
		} catch (Exception e) {
	...
	// 댓글 등록 메서드
	public int insertComment(FoodCommentDTO dto) {
...
//	public CommentDTO selectBySeq(int comment_seq) {
//		String sql = "SELECT * FROM tbl_comment WHERE comment_seq=?";
//		try(Connection con = this.getConnection(); PreparedStatement pstmt = con.prepareStatement(sql);){
//			pstmt.setInt(1, comment_seq);
//			ResultSet rs = pstmt.executeQuery();
//			if(rs.next()) {
...
//				String writer_id = rs.getString("writer_id");
//				CommentDTO dto = new CommentDTO(comment_seq,writer_nickname,comment_content,written_date,board_seq,writer_id);
//				return dto;
//			}
//		}catch(Exception e) {
//			e.printStackTrace();
