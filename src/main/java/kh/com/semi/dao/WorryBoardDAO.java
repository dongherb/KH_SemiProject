...
public class WorryBoardDAO {
	private BasicDataSource bds;

	public WorryBoardDAO() {
		try {
	...
	// 게시판에 글 등록하는 메서드
	public int insert(WorryBoardDTO dto) {
		String sql = "INSERT INTO tbl_board VALUES(board_seq.nextval,?,?,?,sysdate,0,0,'고민',?)";
		try (Connection con = this.getConnection(); PreparedStatement pstmt = con.prepareStatement(sql);) {
	...
	// 게시판 전체 목록 갖고오는 메서드
	public ArrayList<WorryBoardDTO> selectAll() {
		String sql = "SELECT * FROM tbl_board";
		try (Connection con = this.getConnection(); PreparedStatement pstmt = con.prepareStatement(sql);) {

			ResultSet rs = pstmt.executeQuery();
			ArrayList<WorryBoardDTO> list = new ArrayList<>();
			while (rs.next()) {
				...
				list.add(new WorryBoardDTO(board_seq, writer_nickname, board_title, board_content, written_date, view_count,
	...
	public WorryBoardDTO selectBySeq(int board_seq) {
		...
		try (Connection con = this.getConnection(); PreparedStatement pstmt = con.prepareStatement(sql);) {
			...
			if (rs.next()) {
				...
				WorryBoardDTO dto = new WorryBoardDTO(board_seq, writer_nickname, board_title, board_content, written_date,
	...
	public ArrayList<WorryBoardDTO> getBoardList(int startRange, int endRange) {
		String sql = "SELECT * FROM " + "(SELECT ROW_NUMBER() OVER (ORDER BY board_seq desc) 순위,"
				+ "a.* FROM tbl_board a)" + "WHERE (board_category ='고민') AND 순위 BETWEEN ? AND ?";
		...
		try (Connection con = this.getConnection(); PreparedStatement pstmt = con.prepareStatement(sql);) {
			...
			while (rs.next()) {
				...
				int recommended_count = rs.getInt("recommended_count");
				String board_category = rs.getString("board_category");
				list.add(new WorryBoardDTO(board_seq, writer_nickname, board_title, null, written_date, view_count,
