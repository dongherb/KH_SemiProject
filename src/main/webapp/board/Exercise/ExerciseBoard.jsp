	...
	<div class="container">
		...
		<div class="row">
			<div class="col btnWriteDiv">
				...
				<script>
					$("#btn_write").on("click",function(){
						location.href="${pageContext.request.contextPath}/toExerciseWrite.exercisebo";
					})
				</script>
				</c:otherwise>
			</c:choose>
					...
					<tbody>
						<c:forEach items="${list}" var="dto">
							<tr>
								...
								<td><a
									href="${pageContext.request.contextPath}/toExerciseDetailView.exercisebo?board_seq=${dto.getBoard_seq()}&currentPage=${naviMap.get('currentPage')}">${dto.getBoard_title()}</a></td>
			...
			<ul class="pagination justify-content-center">
				<c:if test="${naviMap.get('needPrev') eq true}">
					<li class="page-item"><a class="page-link"
						href="${pageContext.request.contextPath}/toExerciseBoard.exercisebo?currentPage=${naviMap.get('startNavi')-1}">Previous</a></li>
				</c:if>
				<!--startNavi ->endNavi  -->
				<c:forEach var="i" begin="${naviMap.get('startNavi')}"
					end="${naviMap.get('endNavi')}">
					<li class="page-item"><a class="page-link"
						href="${pageContext.request.contextPath}/toExerciseBoard.exercisebo?currentPage=${i}">${i}</a></li>
				</c:forEach>
				<c:if test="${naviMap.get('needNext') eq true}">
					<li class="page-item"><a class="page-link"
						href="${pageContext.request.contextPath}/toExerciseBoard.exercisebo?currentPage=${naviMap.get('endNavi')+1}">Next</a></li>
				</c:if>
			</ul>
		</nav>
	</div>
