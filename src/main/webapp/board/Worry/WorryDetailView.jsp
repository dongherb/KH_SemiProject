...
<div class="gab"></div>
	...
	<form id="WorrymodifyForm"
		action="${pageContext.request.contextPath}/WorrymodifyProc.worrybo"
		method="post">
...
<footer>
	...
	// 추천 버튼 클릭했을 때 이벤트 처리
		...
		btn_recommend.addEventListener("click",function(){
			...
			location.href = "${pageContext.request.contextPath}/WorryRecommendProc.worrybo?board_seq=${dto.getBoard_seq()}&currentPage=${currentPage}";
		});	
	...
	$(document).ready(function(){
		...
		btn_reportSubmit.addEventListener("click",function(){
			location.href="${pageContext.request.contextPath}/WorryreportProc.worryrp?board_seq=${dto.getBoard_seq()}&currentPage=${currentPage}&report_reason="+$("#report_reason").val(); // get 방식으로 넘기기
	...
	btn_complete.addEventListener("click",function(){
		...
		document.getElementById("WorrymodifyForm").submit();
	})
	
	// 게시글 수정 버튼 후 취소 버튼 눌렀을 때 이벤트 처리
	let btn_cancel = document.getElementById("btn_cancel");
	btn_cancel.addEventListener("click",function(){
		location.href = "${pageContext.request.contextPath}/toWorryDetailView.worrybo?board_seq=${dto.getBoard_seq()}&currentPage=${currentPage}";
	})
	...
	$(document).ready(function(){
		...
		$("#btnSubmitCmt").on("click",function(){
			...
			$("#comment_content").val("");
			$.ajax({
				url : "${pageContext.request.contextPath}/WorryinsertProc.worryco",
				type : "post",
				data : data
			}).done(function(rs){
			 if(rs =="success"){
		...
		function getCommentList(){
			...
			$.ajax({
				type: "get",
				url : "${pageContext.request.contextPath}/getWorryCommentProc.worryco?board_seq=${dto.getBoard_seq()}",
				dataType : "json"
			}).done(function(data){
				 $("#cmt_box").empty();
				 console.log("456");
		...
		$(document).on("click",".btn-modifyCmtSuc",function(e){
			...
			$.ajax({
				url:"${pageContext.request.contextPath}/WorrymodifyProc.worryco?comment_seq=" + $(e.target).val() + "&comment_content=" + comment_content ,
				type:"get"
			}).done(function(rs){
				if(rs=="success"){
					getCommentList();
		...
		$(document).on("click","#btn_commentDelete",function(e){
			...
			$.ajax({
				url:"${pageContext.request.contextPath}/WorrydeleteProc.worryco?comment_seq=" + $(e.target).val(),
				type : 'get'
			}).done(function(rs){
				if(rs=="success"){
